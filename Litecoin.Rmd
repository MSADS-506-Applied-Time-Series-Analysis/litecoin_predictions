...
```
From the correlation table, it can be discerned that whereas the OHLC and adjusted prices exhibit multicollinearity at $r=1,$ their relationships with volume is much less pronounced, where $0.56 \leq r \leq 0.58.$
...
\begin{eqnarray*}
\rho x(h) &=& \frac{\gamma(x(h)}{\gamma x(0)} = \frac{(X_{t+h}-\bar{X}) (X_t-\bar{x})}{\large \sum (X_t-\bar{X})^2}\\
\\
&=&\text{Corr}(X_{t+h,}X_t)
\end{eqnarray*}
